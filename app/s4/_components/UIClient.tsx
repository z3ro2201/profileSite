...
export default function UIClient() {
  ...
  const dismissToast = (i: number) => setToasts((prev) => prev.map((v, idx) => (idx === i ? false : v)));
  ...
  const badgeVariants = [
    ...
    { label: "muted", style: { background: "var(--secondary)", color: "var(--muted-foreground)" } as CSSProperties },
    {
      label: "outline",
      style: {
  ...
  const btnVariants = [
    ...
    {
      label: "secondary",
      style: { background: "var(--secondary)", color: "var(--foreground)" } as CSSProperties,
      cls: "border-transparent",
    },
    {
      label: "outline",
      style: { background: "transparent", color: "var(--foreground)" } as CSSProperties,
      ...
      <div className="space-y-0">
          ...
          <div className="space-y-3">
            {badgeVariants.map((v) => (
              <div key={v.label} className="flex flex-wrap items-center gap-3">
                <span className="w-14 text-[10px] text-muted-foreground flex-shrink-0" style={mono}>
                  {v.label}
                </span>
                {badgeSizes.map((s) => (
                  <span key={s.label} className={s.cls} style={{ ...mono, ...v.style }}>
                    {s.label}
                  </span>
                ))}
              </div>
            ...
            {btnVariants.map((v) => (
              <div key={v.label} className="flex flex-wrap items-center gap-3">
                ...
                {btnSizes.map((s) => (
                  <button
                    key={s.label}
                    className={`${s.cls} ${v.cls} font-medium transition-opacity hover:opacity-80`}
                    style={{ ...sans, ...v.style }}
                  >
                    {s.label}
                  </button>
          ...
          <div className="flex flex-wrap items-center gap-6">
            {cbSizes.map((s) => (
              <label key={s.label} className={`flex items-center gap-2 cursor-pointer select-none ${s.textCls}`}>
                <span
                  className="flex-shrink-0 rounded flex items-center justify-center border transition-colors"
                  style={{
                    width: s.size,
                    height: s.size,
                    background: s.val ? TEAL : "transparent",
                    borderColor: s.val ? TEAL : "var(--border)",
                  }}
                  onClick={() => s.set(!s.val)}
                >
                  {s.val && (
          ...
          <div className="space-y-3">
            {radioSizes.map((sz) => (
              <div key={sz.label} className="flex flex-wrap items-center gap-6">
                <span className="w-8 text-[10px] text-muted-foreground flex-shrink-0" style={mono}>
                  {sz.label}
                </span>
                {radioOptions.map((opt) => (
                  <label key={opt} className={`flex items-center gap-2 cursor-pointer select-none ${sz.textCls}`}>
                    <span
                      ...
                      style={{ width: sz.size, height: sz.size, borderColor: radio === opt ? TEAL : "var(--border)" }}
                      onClick={() => setRadio(opt)}
                    >
                      {radio === opt && (
                        <span
                          className="rounded-full"
                          style={{ width: sz.size - 6, height: sz.size - 6, background: TEAL }}
                        />
                      )}
                    </span>
              ...
              <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 pointer-events-none">
                ...
                >
                  <div className="flex items-center justify-between px-6 py-4 border-b border-border">
                    ...
                    <button
                      onClick={() => setModalOpen(false)}
                      className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-black/6"
                    >
                  ...
                  <div className="px-6 py-5">
                    <p className="text-sm text-muted-foreground leading-relaxed font-light">
                      이것은 모달 본문 텍스트입니다. 확인 또는 취소 버튼으로 닫을 수 있습니다.
                    </p>
                  </div>
                  <div className="flex justify-end gap-2 px-6 pb-5">
                    <button
                      onClick={() => setModalOpen(false)}
                      className="text-sm px-4 py-2 rounded-full border border-border hover:bg-black/4 transition-colors"
                      style={sans}
                    >
                      취소
                    </button>
                    <button
                      onClick={() => setModalOpen(false)}
                      className="text-sm px-4 py-2 rounded-full text-white transition-opacity hover:opacity-80"
                      style={{ background: TEAL, ...sans }}
                    >
                      확인
                    </button>
              ...
              <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 pointer-events-none">
                ...
                >
                  ...
                  <div className="flex gap-2 px-6 pb-5">
                    <button
                      onClick={() => setAlertOpen(false)}
                      className="flex-1 text-sm px-4 py-2 rounded-full border border-border hover:bg-black/4 transition-colors"
                      style={sans}
                    >
                      취소
                    </button>
                    <button
                      onClick={() => setAlertOpen(false)}
                      className="flex-1 text-sm px-4 py-2 rounded-full text-white transition-opacity hover:opacity-80"
                      style={{ background: "#ef4444", ...sans }}
                    >
                      삭제
                    </button>
                  </div>
            ...
            {[
              {
                tag: "h1",
                el: (
                  <h1 style={{ ...serif, fontSize: "3rem", fontWeight: 300, lineHeight: 1.1 }}>
                    아름다운 타이포그래피
                  </h1>
                ),
              },
              {
                tag: "h2",
                el: (
                  <h2 style={{ ...serif, fontSize: "2.25rem", fontWeight: 300, fontStyle: "italic", lineHeight: 1.15 }}>
                    섬세한 디자인의 교차점
                  </h2>
                ),
              },
              ...
              },
              {
                ...
                el: (
                  <p className="text-base text-muted-foreground font-light leading-relaxed">
                    사람들이 실제로 즐겨 쓰는 제품을 만들기 위해, 깔끔한 엔지니어링과 섬세한 디자인의 교차점에서
                    일합니다.
                  </p>
                ),
              },
              ...
              <div
                key={tag}
                className={`flex items-start gap-4 py-4 ${i < arr.length - 1 ? "border-b border-border" : ""}`}
              >
                <span className="text-[10px] text-muted-foreground w-14 flex-shrink-0 pt-1" style={mono}>
                  {tag}
                </span>
                ...
                <div className="flex flex-wrap gap-2">
                  {colors.map((c) => (
                    <span
                      ...
                      style={{ ...mono, background: c.badge.bg, color: c.badge.text }}
                    >
                      {c.name}
                    </span>
                  ))}
        ...
        <section className="py-8 border-b border-border">
          ...
          <div className="flex flex-wrap gap-3">
            {/* Default */}
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 min-w-[200px] max-w-[260px] flex flex-col gap-3">
              <div>
              ...
              <button
                className="text-xs px-3 py-1.5 rounded-full text-white self-start"
                style={{ background: TEAL, ...sans }}
              >
                버튼
              </button>
            ...
            {/* Teal accent */}
            <div
              className="rounded-2xl border border-[var(--border)] p-5 min-w-[200px] max-w-[260px]"
              style={{ borderLeft: `3px solid ${TEAL}`, background: BG }}
            >
            ...
            <div className="rounded-2xl p-5 min-w-[200px] max-w-[260px]" style={{ background: "#1a1a16" }}>
              ...
              <p className="text-xs leading-relaxed font-light" style={{ color: "#a0a09a" }}>
                어두운 배경 위의 카드입니다.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1" style={mono}>
            {["첫화면", "블로그", "글 제목"].map((item, i, arr) => (
              <span key={item} className="flex items-center gap-1">
                <span
                  className={`text-xs ${i === arr.length - 1 ? "" : "text-muted-foreground hover:text-foreground cursor-pointer"}`}
        ...
        <section className="py-8 border-b border-border">
          ...
          <div className="mb-2">
            <span
              ...
              style={{ ...mono, background: "rgba(59,130,246,0.12)", color: "#3b82f6" }}
            >
              ...
              <thead>
                <tr style={{ background: "var(--secondary)" }}>
                  {["이름", "역할", "기술", "경력", "상태", "액션"].map((h) => (
                    <th
                      key={h}
                      className="text-left px-4 py-3 text-[10px] uppercase tracking-widest text-muted-foreground font-medium"
                      style={mono}
                    >
                      {h}
                    </th>
                ...
                {[
                  {
                    ...
                    exp: "5년",
                    status: "재직 중",
                    statusColor: { bg: "rgba(35,198,169,0.12)", text: "#23c6a9" },
                  },
                  {
                    name: "이서연",
                    role: "Backend Dev",
                    ...
                    exp: "3년",
                    status: "협업 중",
                    statusColor: { bg: "rgba(245,158,11,0.12)", text: "#d97706" },
                  },
                  {
                    name: "박민준",
                    role: "DevOps",
                    ...
                    exp: "7년",
                    status: "휴직",
                    statusColor: { bg: "rgba(239,68,68,0.12)", text: "#ef4444" },
                  },
                ].map((row, i) => (
                  <tr key={i} className="border-t border-border hover:bg-black/[0.02] transition-colors">
                    ...
                    <td className="px-4 py-3 text-muted-foreground text-xs" style={mono}>
                      {row.tech}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span
                        ...
                        style={{ ...mono, background: row.statusColor.bg, color: row.statusColor.text }}
                      >
                        {row.status}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <button
                        className="text-[10px] px-2.5 py-1 rounded-full border border-border text-muted-foreground hover:border-[#23c6a9] hover:text-[#23c6a9] transition-colors"
                        style={mono}
                      >
                        보기
                      </button>
          ...
          <div className="flex flex-wrap items-end gap-6">
            {[
              ...
              <div key={s.label} className="flex flex-col items-center gap-2">
                <div
                  className={`${s.cls} rounded-full animate-spin border-t-transparent`}
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={p.pct} className="flex items-center gap-3">
                <span className="text-[10px] text-muted-foreground w-8 text-right flex-shrink-0" style={mono}>
                  {p.label}
                </span>
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={r.label} className="flex items-center gap-4">
                <span className="text-[10px] text-muted-foreground w-8 flex-shrink-0" style={mono}>
                  {r.label}
                </span>
                <input
                  type="range"
                  ...
                  style={{
                    accentColor: TEAL,
                    background: `linear-gradient(to right, ${TEAL} ${r.value}%, var(--border) ${r.value}%)`,
                  }}
                />
                <span className="text-[10px] text-muted-foreground w-8 text-right flex-shrink-0" style={mono}>
                  {r.value}
            ...
            <div className="flex">
              {["이전", "현재", "다음"].map((label, i) => (
                <button
                  ...
                  style={{
                    ...mono,
                    borderColor: i === 1 ? TEAL : "var(--border)",
        ...
        <section className="py-8 border-b border-border">
          ...
          <div className="text-sm text-muted-foreground font-light leading-relaxed">
            {activeTab === "개요" &&
              "풀스택 개발자로 사용자 경험을 중심에 두고 제품을 만듭니다. 아이디어에서 배포까지 전 과정을 경험했습니다."}
            {activeTab === "기술" &&
              "TypeScript · React · Next.js · Node.js · PostgreSQL · Docker · AWS · Tailwind CSS 등을 주력으로 사용합니다."}
            {activeTab === "경력" &&
              "Craft Labs (2023–현재), Novaline Systems (2021–2023), Pixel Workshop (2019–2021) 등에서 근무했습니다."}
          </div>
        </section>
            ...
            {[
              {
                ...
                a: "TypeScript와 React를 기반으로 프론트엔드를 구성하고, Node.js와 Python으로 백엔드를 처리합니다. 인프라는 AWS와 Docker를 주로 활용합니다.",
              },
              {
                q: "프리랜서 프로젝트를 받으시나요?",
                a: "네, 흥미로운 프로젝트라면 협업할 수 있습니다. 먼저 이메일로 간단한 프로젝트 개요를 보내주시면 검토 후 답변드리겠습니다.",
              },
              {
                q: "협업 방식은 어떻게 되나요?",
                a: "비동기 커뮤니케이션을 선호하며, 주로 GitHub과 Notion을 통해 협업합니다. 필요한 경우 정기 미팅도 진행합니다.",
              },
            ].map(({ q, a }, i) => (
              <div key={i} className={i > 0 ? "border-t border-border" : ""}>
                ...
                {accordionOpen.includes(i) && (
                  <div
                    className="px-5 pb-4 text-sm text-muted-foreground font-light leading-relaxed border-t border-border pt-3"
                    style={sans}
                  >
                    {a}
                  </div>
        ...
        <section className="py-8 border-b border-border">
          ...
          <div className="space-y-2 max-w-sm">
            {[
              ...
              ({ icon: Icon, bg, text, msg, iconColor }, i) =>
                toasts[i] && (
                  <div key={i} className="flex items-center gap-3 rounded-xl p-3" style={{ background: bg }}>
                    ...
                    <span className="flex-1 text-sm font-medium" style={{ color: text, ...sans }}>
                      {msg}
                    </span>
                    <button
            ...
            {toasts.every((v) => !v) && (
              <button
                onClick={() => setToasts([true, true, true, true])}
        ...
        <section className="py-8 border-b border-border">
          ...
          <div className="relative inline-block">
            ...
            {popoverOpen && (
              <div
                className="absolute left-0 top-full mt-2 z-50 w-56 rounded-2xl border border-border shadow-lg p-4"
                style={{ background: BG }}
              >
                ...
                <p className="text-xs text-muted-foreground font-light leading-relaxed">
                  버튼 아래에 표시되는 작은 팝오버 패널입니다. 추가 정보나 빠른 액션을 담을 수 있어요.
                </p>
              </div>
            )}
          </div>
                ...
                style={
                  active
                    ? {
                        ...
                        background: "rgba(35,198,169,0.08)",
                        color: TEAL,
