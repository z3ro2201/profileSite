...
import type { MaterialChartProp, GemChartResponse, GemChartRow, Tab } from "@/types/Lostark";
...
// 재료는 "N레벨 OOO의 보석" 같은 접두사 조합이 없음 — 재료명 그대로가 item_name
const buildApiPath = (itemName: string, updatetime: string) => {
  const base = `/api/app/game/onstove/lostark/market-chart/material/${encodeURIComponent(itemName)}`;
...
const MaterialChartClient: React.FC<MaterialChartProp> = ({ MATERIAL_ITEM_LIST, initialItemName, UPDATETIME_LIST, initialData, initialUpdatetime }) => {
  const router = useRouter();
  const sp = useSearchParams();

  const updatetimeParam = sp.get("updatetime") ?? undefined;
  const firstItemFromList = (MATERIAL_ITEM_LIST?.[0] ?? "").trim();

  const fallbackUp = clampUpdatetime(updatetimeParam ?? initialUpdatetime ?? UPDATETIME_LIST?.[0] ?? DEFAULT_UPDATETIME);

  const [itemName, setItemName] = useState<string>(initialItemName ?? firstItemFromList);
  ...
  const updateQuery = (item: string, u: string) => {
    const params = new URLSearchParams(sp.toString());
    params.set("updatetime", clampUpdatetime(u));
    router.replace(`/tools/game/onstove/lostark/market-chart/material/${encodeURIComponent(item)}?${params.toString()}`, { scroll: false });
  };

  const effectiveUp = clampUpdatetime(updatetimeParam ?? updatetime ?? DEFAULT_UPDATETIME);

  useEffect(() => {
    updateQuery(itemName, effectiveUp);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [itemName, firstItemFromList]);
  ...
  const apiPath = useMemo(() => {
    return buildApiPath(itemName, effectiveUp);
  }, [itemName, effectiveUp]);

  const ready = !!effectiveUp && !!itemName;
...
export default MaterialChartClient;
