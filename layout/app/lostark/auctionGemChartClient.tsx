...
export default function AuctionGemChartClient({ GEMSTONE_LIST, initialGemStone, initialLevel, UPDATETIME_LIST, initialUpdatetime }: Prop) {
  ...
  useEffect(() => {
    ...
    const needPatch = !updatetimeParam;
    if (needPatch) updateQuery(gemStone, level, u);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const apiPath = useMemo(() => {
    return buildApiPath(initialGemStone ?? "작열", initialLevel ?? "10", effectiveUp);
  }, [effectiveUp]);
              ...
              <ol className="flex gap-2 border-b border-black/10 px-2 pt-2 mb-3">
                <li>
                  <button
                    ...
                    className={["px-3 py-2 cursor-pointer text-[.85rem] rounded-t-lg transition", activeTab === "CHANGE" ? "bg-white text-gray-900 shadow-sm border border-black/10" : "text-gray-700 hover:text-gray-900"].join(" ")}
                  >
                    변동 목록
                  </button>
                </li>
                <li>
                  <button
                    type="button"
                    onClick={() => setActiveTab("OPEN_API")}
                    className={["px-3 py-2 cursor-pointer text-[.85rem] rounded-t-lg transition", activeTab === "OPEN_API" ? "bg-white text-gray-900 shadow-sm border border-black/10" : "text-gray-700 hover:text-gray-900"].join(" ")}
                  >
                    OpenAPI
                  </button>
                </li>
                          ...
                          return (
                            <li key={i} className="flex justify-between items-center">
                              ...
                              <div className="flex gap-3 items-center">
                                ...
                                <span className={["font-mono w-[80px] text-[.8rem] text-right", up ? "text-red-600" : "", down ? "text-blue-600" : "", !up && !down ? "opacity-50" : ""].join(" ")}>
                                  {r.diff == null ? "" : up ? `+${r.diff.toLocaleString()}` : r.diff.toLocaleString()}
                                </span>
                              </div>
                            </li>
                          );
