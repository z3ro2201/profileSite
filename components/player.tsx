...
const Player = ({ onBgImageChange }: PlayerProps) => {
  ...
  const [playerViewMode, setPlayerViewMode] = useState<boolean>(true);
  ...
  return (
    <div className="z-[1000] fixed select-none touch-none" style={{ right: playerViewMode ? -x : 10, bottom: playerViewMode ? -y : 10 }} onPointerMove={onPointerMove} onPointerUp={onPointerUp}>
      {/* drag header */}
      {playerViewMode ? (
        <div className="flex items-center gap-2 px-3 py-2 border-2 border-slate-200 bg-slate-800 overflow-hidden shadow-md text-white rounded cursor-grab active:cursor-grabbing" onPointerDown={onPointerDown} role="dialog">
          ...
          <button type="button" className="cursor-pointer" onPointerDown={(e) => e.stopPropagation()} onClick={toggleView}>
        ...
        <button
          type="button"
          className="flex items-center gap-2 px-3 py-2 border-2 border-slate-200 bg-slate-800 overflow-hidden shadow-md text-white rounded-full cursor-grab cursor-pointer active:cursor-grabbing"
          onPointerDown={(e) => e.stopPropagation()}
          onClick={toggleView}
          title="음악 플레이어를 보이도록 합니다."
        >
      ...
      <div className={cn(playerViewMode ? "videoPlayer" : "hidden")} style={playerContainerStyle} role="application">
